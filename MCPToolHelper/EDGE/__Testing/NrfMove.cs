...
internal class NrfMove : IExternalCommand
{
    ...
    using (Transaction transaction = new Transaction(document, "Move From Reference"))
    {
      try
      {
        ...
        if (!elementIds.Any<ElementId>())
          elementIds = References.PickNewReferences("Select the elements to move");
        if (!elementIds.Any<ElementId>())
          return (Result) 1;
        ...
        for (int index = 0; index < strArray.Length; ++index)
        {
          ...
          double result3;
          double.TryParse(match.Groups["inches_whole"].ToString(), out result3);
          if (match.Groups["inches_part"].Success)
          {
