...
// Type: EDGE.SelectionAndPinningTools.LevelsUnpinAll
...
internal class LevelsUnpinAll : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    ICollection<ElementId> elementIds = Components.GetAllLevels(document).ToElementIds();
    if (!ModelLockingUtils.ShowPermissionsDialog(document, ModelLockingToolPermissions.PinUnpin))
      return (Result) 1;
    using (Transaction transaction = new Transaction(document, "Unpin All Levels"))
    {
      try
      {
