...
// Type: EDGE.TicketTools.TicketPopulator.PopulatorForm
...
public class PopulatorForm : System.Windows.Forms.Form
{
  ...
  public PopulatorForm(Document doc)
  {
  ...
  private bool ValidateData()
  {
    ...
    if (this.cmbScale.SelectedIndex == -1)
    {
      ...
      else if (!int.TryParse(this.cmbScale.Text, out int _) && !this.cmbScale.Text.Equals("<No Scale Required>"))
      {
  ...
  private void btnPopulate_Click(object sender, EventArgs e)
  {
    ...
    if (this.cmbScale.SelectedIndex == -1)
    {
      ...
      else if (this.cmbScale.Text.Equals("<<Varies>>"))
        this.SelectedScaleFactor = -1;
  ...
  private void FillTemplateData(string templateName)
  {
    ...
    if (source.Any<TicketTemplate>())
    {
      ...
      bool flag = true;
      foreach (AnchorGroup anchorGroup in template.AnchorGroups)
      {
        if (anchorGroup.MainView != null && anchorGroup.MainView.originalScale != template.TemplateScale)
          flag = false;
        if (anchorGroup.TopView != null && anchorGroup.TopView.originalScale != template.TemplateScale)
          flag = false;
        if (anchorGroup.BottomView != null && anchorGroup.BottomView.originalScale != template.TemplateScale)
          flag = false;
        if (anchorGroup.LeftView != null && anchorGroup.LeftView.originalScale != template.TemplateScale)
          flag = false;
        if (anchorGroup.RightView != null && anchorGroup.RightView.originalScale != template.TemplateScale)
          flag = false;
      }
      int num1 = !flag ? -1 : template.TemplateScale;
      this.ScaleUnits = template.ScaleUnits;
  ...
  private PopulatorForm.SizeWarning CheckSelectedScaleAgainstBBox(int scale)
  {
    ...
    return (flag ? num6 : num7) > num3 | (flag ? num7 : num6) > num2 ? PopulatorForm.SizeWarning.Error : PopulatorForm.SizeWarning.OK;
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.label5.Size = new Size(49, 16 /*0x10*/);
    ...
    this.lblConstructionProduct.Size = new Size(44, 16 /*0x10*/);
    this.lblConstructionProduct.TabIndex = 9;
    this.lblConstructionProduct.Text = "label6";
    this.button1.Location = new System.Drawing.Point(163, 300);
    ...
    this.Name = nameof (PopulatorForm);
