...
const DEFAULT_BASE_URL =
  process.env.REVIT_2025_BRIDGE_URL?.trim() || "http://127.0.0.1:8767";
...
async function postJson<T>(route: string, payload: unknown): Promise<T> {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      throw new Error(
        `Gateway request to ${route} timed out after ${REQUEST_TIMEOUT_MS / 1000}s (no response from Revit).`,
      );
    }
    throw new Error(
...
// The gateway writes the full snapshot to disk (C#-side); only a small summary
// ({ outputPath, elementCount, byteSize, errorCount }) returns through MCP.
export async function exportInstances(input: JsonObject): Promise<JsonObject> {
  return postJson<JsonObject>("/api/project/export_instances", input);
}

export async function exportTypeParameters(input: JsonObject): Promise<JsonObject> {
  return postJson<JsonObject>("/api/project/export_type_parameters", input);
}
...
export async function exportMaterials(input: JsonObject): Promise<JsonObject> {
  return postJson<JsonObject>("/api/project/export_materials", input);
}

export async function exportViews(input: JsonObject): Promise<JsonObject> {
  return postJson<JsonObject>("/api/project/export_views", input);
}
