...
// Type: EDGE.VisibilityTools.ProductVisibility_Opaque
...
internal class ProductVisibility_Opaque : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    if (Utils.ViewUtils.ViewUtils.CurrentViewIsProjectBrowser(document))
      return (Result) 1;
    bool flag = false;
    using (Transaction transaction = new Transaction(document, "PRECAST PRODUCT - Opaque"))
    {
      try
      {
        ...
        if (!flag)
          ProductVisibility_Opaque.AddFilter();
        document.Regenerate();
        foreach (ElementId filter in (IEnumerable<ElementId>) document.ActiveView.GetFilters())
        {
          ...
          if (element.Name.ToUpper().Contains("PRECAST PRODUCT"))
          {
            ...
            filterOverrides.SetSurfaceTransparency(0);
