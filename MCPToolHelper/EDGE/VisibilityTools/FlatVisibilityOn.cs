...
// Type: EDGE.VisibilityTools.FlatVisibilityOn
...
internal class FlatVisibilityOn : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    using (Transaction transaction = new Transaction(document, "Turn ON Flat"))
    {
      try
      {
        ...
        {
          if ((document.GetElement(filter) as ParameterFilterElement).Name.Contains("FLAT PRODUCT"))
          {
            flag = true;
            break;
          }
        ...
        if (!flag)
          FlatVisibilityOn.AddFlatFilter(document);
        document.Regenerate();
        foreach (ElementId filter in (IEnumerable<ElementId>) document.ActiveView.GetFilters())
        {
          if ((document.GetElement(filter) as ParameterFilterElement).Name.ToUpper().Contains("FLAT PRODUCT"))
          {
  ...
  private static void AddFlatFilter(Document doc)
  {
    ...
    new TaskDialog("Warning")
    {
      ...
      MainInstruction = "Warning:  There is no filter for \"FLAT PRODUCT\" available to turn on. Make sure that the filter is both created and enabled in your Visibility/Graphics settings and try again."
    }.Show();
  }
}
