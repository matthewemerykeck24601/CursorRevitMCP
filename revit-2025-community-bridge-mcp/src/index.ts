...
process.on("unhandledRejection", (reason) => {
  process.stderr.write(
    `revit-2025-community-bridge-mcp unhandledRejection: ${
...
process.on("uncaughtException", (error) => {
  process.stderr.write(
    `revit-2025-community-bridge-mcp uncaughtException: ${error.stack ?? error.message}\n`,
  );
});
...
main().catch((error) => {
  process.stderr.write(`revit-2025-community-bridge-mcp failed: ${String(error)}\n`);
  process.exit(1);
});
