...
// Type: EDGE.VisibilityTools.WarpedVisibilityOff
...
internal class WarpedVisibilityOff : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    using (Transaction transaction = new Transaction(document, "Turn OFF Warped"))
    {
      try
      {
        ...
        {
          if ((document.GetElement(filter) as ParameterFilterElement).Name.Contains("WARPED PRODUCT"))
          {
            flag = true;
            break;
          }
        ...
        if (!flag)
          WarpedVisibilityOff.AddWarpedFilter(document);
        document.Regenerate();
        foreach (ElementId filter in (IEnumerable<ElementId>) document.ActiveView.GetFilters())
        {
  ...
  private static void AddWarpedFilter(Document doc)
  {
    ...
    new TaskDialog("Warning")
    {
      ...
      MainInstruction = "Warning:  There is no filter for \"WARPED PRODUCT\" available to turn off. Make sure that the filter is both created and enabled in your Visibility/Graphics settings and try again."
    }.Show();
  }
}
