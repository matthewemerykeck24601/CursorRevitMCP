...
internal class ResetCPH : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    using (Transaction transaction = new Transaction(document, "Reset CPH Parameters"))
    {
      try
      {
        if (new TaskDialog("Reset CONSTRUCTION_PRODUCT_HOST Parameters.")
        {
          MainInstruction = "Are you sure you want to rest CONSTRUCTION_PRODUCT_HOST Parameter values for all Elements in the model?",
        ...
        new TaskDialog("ResetCPH Parameters")
        {
          FooterText = "EDGE^R 2024 12.2.0 (build date: 2025.06.10)",
