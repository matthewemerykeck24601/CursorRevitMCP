...
// Marshals work onto Revit's API thread via an ExternalEvent. HTTP handler threads call
// Enqueue(...) and await the result; Execute(app) drains the queue on the Revit thread.
// Identical in shape to the 2025 bridge dispatcher (no ElementId usage, so no porting).
public sealed class RevitRequestDispatcher : IExternalEventHandler
{
    ...
    public string GetName() => "RevitDiag2024Bridge.RequestDispatcher";

    public Task<T> Enqueue<T>(Func<UIApplication, T> action, int timeoutMs = 15000)
    {
        var tcs = new TaskCompletionSource<object?>(TaskCreationOptions.RunContinuationsAsynchronously);
        var request = new QueuedRequest(app => action(app), tcs);
        lock (_lock)
        {
    ...
    private static async Task<T> WaitWithTimeout<T>(QueuedRequest request, int timeoutMs)
    {
        ...
        // real result so callers never retry work that is still executing.
        var raw = await task;
