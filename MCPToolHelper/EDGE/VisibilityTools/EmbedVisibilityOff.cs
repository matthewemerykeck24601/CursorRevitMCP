...
// Type: EDGE.VisibilityTools.EmbedVisibilityOff
...
internal class EmbedVisibilityOff : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    if (Utils.ViewUtils.ViewUtils.CurrentViewIsProjectBrowser(document))
      return (Result) 1;
    bool flag = false;
    using (Transaction transaction = new Transaction(document, "Turn OFF Embeds"))
    {
      try
      {
        ...
        if (!flag)
          EmbedVisibilityOff.AddFilter(document);
        document.Regenerate();
        foreach (ElementId filter in (IEnumerable<ElementId>) document.ActiveView.GetFilters())
        {
  ...
  private static void AddFilter(Document doc)
  {
    ...
    new TaskDialog("Warning")
    {
      ...
      MainInstruction = "Warning:  There is no filter for \"EMBED\" available to turn off. Make sure that the filter is both created and enabled in your Visibility/Graphics settings and try again."
    }.Show();
  }
}
