...
/** Validates a native/iOS Bearer access token against APS before trusting it as a session. */
export async function validateApsAccessToken(accessToken: string): Promise<void> {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Failed APS bearer validation: ${response.status} ${text}`);
  }
}
