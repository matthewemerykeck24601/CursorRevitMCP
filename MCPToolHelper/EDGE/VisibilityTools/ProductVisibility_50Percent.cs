...
// Type: EDGE.VisibilityTools.ProductVisibility_50Percent
...
internal class ProductVisibility_50Percent : IExternalCommand
{
  public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
  {
    ...
    if (Utils.ViewUtils.ViewUtils.CurrentViewIsProjectBrowser(document))
      return (Result) 1;
    bool flag = false;
    using (Transaction transaction = new Transaction(document, "PRECAST PRODUCT - 50% Transparent"))
    {
      try
      {
        ...
        if (!flag)
          ProductVisibility_50Percent.AddFilter();
        document.Regenerate();
        foreach (ElementId filter in (IEnumerable<ElementId>) document.ActiveView.GetFilters())
        {
          ...
          if (element.Name.ToUpper().Contains("PRECAST PRODUCT"))
          {
            ...
            filterOverrides.SetSurfaceTransparency(50);
